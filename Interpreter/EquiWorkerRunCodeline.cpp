...
{
	vector<stack<pair<EquiObject*, bool>>> pReg;
	while (lineCount < code->size() && pLine != lineCount)
	{
		...
		try
		{
			...
			else if (ln.cmd == EC_FUNCTION_CALL && !breakFlag && !continueFlag)
			{
				...
				{
					if (resF == -1)
						out = (*func.first)(input.first, f->apparentTokens());
					else
					{
						out = globalCore->getResult(resF);
						resF = -1;
					}
				}
				catch(pair<EquiTask*, int> o)
				{
		            int resAt = globalCore->addPost(o.second, o.first);
		            resF = resAt;
		            throw resAt;
				}
				
				pair<EquiObject*, bool> o(out, true);
			...
			else if (ln.cmd == EC_INVOKE_TASK && !breakFlag && !continueFlag)
			{
				...
				{
					EquiObject* out = (*func.first)(input.first, f->apparentTokens());
					EquiFuture* fut = new EquiFuture;
					fut->setID(globalCore->addResult(out));
					pair<EquiObject*, bool> o(fut, true);
					registers[ln.reg].push(o);
				}
			...
			else if (ln.cmd == EC_SPAWN && !breakFlag && !continueFlag)
			{
				...
				EquiObject* newObj = typeDef->spawnMyType();
				//cout << "SpawningC" << endl;
		...
		catch (int n)
		{
			if (!mainWorker)
			{
				EquiTask* frozen = new EquiTask;
				frozen->lineCount = lineCount;
				frozen->scopeSince = scopeSince;
				frozen->breakFlag = breakFlag;
				frozen->continueFlag = continueFlag;
				frozen->elseFlag = elseFlag;
				frozen->frame = data;
				frozen->registers = pReg;
				frozen->code = code;
				frozen->resumeAt = resF;

				pair<EquiTask*, int> o(frozen, n);
	...
	pair<EquiObject*, bool> p(new EquiVoid, true);
	return p;
}
